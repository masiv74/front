...
export const ViewRegistrosCel = () => {
  ...
  useEffect(() => {
    const fetchUsuarios = async () => {
      try {
        const response = await ppcApi.get("/getadmin/spadmin/datos");
        if (response.data.ok) {
  ...
  return (
    <>
      <div className="container mx-auto px-4 sm:px-8">
        <div className="py-8">
          <h2 className="text-2xl leading-tight mb-4">Ver Registros de Vecinos</h2>
                ...
                {memoizedData.length > 0 ? (
                  memoizedData.map((registro, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell align="center">{registro.ndocumento}</TableCell>
                      <TableCell align="center">{registro.recolector}</TableCell>
                    </TableRow>
                  ))
                ) : (
                  <TableRow>
